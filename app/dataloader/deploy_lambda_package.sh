...
aws s3 cp MovieDataLoaderFunction.zip s3://$(aws cloudformation describe-stacks --stack-name core-infrastructure-function-bucket --query "Stacks[0].Outputs[?OutputKey=='BucketName'].OutputValue" --output text)

aws lambda update-function-code \
--function-name $(aws cloudformation describe-stacks --stack-name core-infrastructure-movie-data-app --query "Stacks[0].Outputs[?OutputKey=='MovieDataLoaderFunction'].OutputValue" --output text) \
